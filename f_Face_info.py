...
def get_face_info(im):
    ...
    if len(boxes_face)!=0:
        for box_face in boxes_face:
            ...
            face_features["age"] = str(round(age))
...
def bounding_box(out,img):
    ...
    for data_face in out:
        ...
        if len(box) == 0:
            ...
            try:
                cv2.putText(img, "age: " +data_face["age"], (x0, y0-7), cv2.FONT_HERSHEY_DUPLEX, fontSize, (38,0,237), thickness)
            except:
                pass
            try:
                cv2.putText(img, "gender: " +data_face["gender"], (x0, y0-step-10*1), cv2.FONT_HERSHEY_DUPLEX, fontSize, (38,0,237), thickness)
            except:
                pass
            try:
                cv2.putText(img, "emotion: " +data_face["emotion"], (x0, y0-step-10*2), cv2.FONT_HERSHEY_DUPLEX, fontSize, (38,0,237), thickness)
            except:
                pass
            try:
                cv2.putText(img, "name: " +data_face["name"], (x0, y0-step-10*3), cv2.FONT_HERSHEY_DUPLEX, fontSize, (38,0,237), thickness)
            except:
                pass
    return img
